defmodule ApiGateway.ProductClient do
  ...
  def init(_opts) do
    ...
    Supervisor.init(children,
      ...
      restart: :transient,
      name: ApiGateway.ProductClientSupervisor
    )
  end
  ...
  def run(query, variables \\ %{}, context \\ %{}) do
    ...
    result =
      ...
      |> Broker.publish(:product_graphql_request)
      |> case do
        {:ok, _} ->
          Logger.info("Sent #{request_id} product request")

          receive do
            response ->
              Logger.info("Received #{request_id} product request")

              {:ok, response}
          after
            response_timeout() ->
              Logger.info("Could not wait for #{request_id} product request")

              {:error, :response_timeout}
          end

        _ ->
          Logger.info("Could not send #{request_id} product request")
      end
